...
def load_data_BS(dataset):
    ...
    train_set_x = train_set[:21,:]
    #train_set_x = train_set[243:257,:] 
    test_set_x = train_set[21:,:]
...
def main(argv):
    ...
    #data_train_0,data_train_1,data_train_2, data_test = load_data_BS("data/changedetection\\camerajitter\\badminton/orignal_color.pkl")
    img = Image.open("data/changedetection\camerajitter\\badminton/gt001060.png")
    ...
    for k in range(4):
        ...
        for i in range(batches):
            global batch_no
            batch_no = i
            os.path.walk('C:\work\\backgdSubt\dataset\changedetection\camerajitter\\badminton\\orignal', load_batch, 0)
            global train_set_x0
            global train_set_x1
            global train_set_x2
    ...
    os.path.walk('C:\work\\backgdSubt\dataset\changedetection\camerajitter\\badminton\\test', load_test_batch, 0)
    ...
    for point in xrange(Ndat):
        ...
        #dataout = vstack((dataout,X.flatten()))
        dataout = vstack((dataout,subtract(asarray(X),data_test_0[:,-1],asarray(dbn[0].vsigma),point+5)))
    ...
    for point in xrange(Ndat):
        ...
        #print "S hsape:", X.shape
        #dataout1 = vstack((dataout1,X.flatten()))
        dataout1 = vstack((dataout1,subtract(asarray(X),data_test_1[:,-1],asarray(dbn1[0].vsigma),point+5)))
    ...
    for point in xrange(Ndat):
        ...
        #print "S hsape:", X.shape
        #dataout2 = vstack((dataout2,X.flatten()))
        dataout2 = vstack((dataout2,subtract(asarray(X),data_test_2[:,-1],asarray(dbn2[0].vsigma),point+5)))
    ...
    #plt.figure(2)
    plt.show()
    srcDir = "C:\work\\backgdSubt\dataset\datasets\change detection\cameraJitter\\badminton\input"
    targetDir = "C:\work\\backgdSubt\dataset\datasets\change detection\\cameraJitter\\badminton\grbm"

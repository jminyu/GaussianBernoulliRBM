...
def main(argv):
    ...
    data_train_0,data_train_1,data_train_2, data_test = load_data_BS("data/Sabs\\basic/orignal_color.pkl")
    img = Image.open("data/Sabs\\basic/gt0139.png")
    print "Doie : " , (asarray(img)[:,:,0]).shape
    groundtruth = (((asarray(img)[:,:,2])/255.0 > 0.5) * 1).flatten()
    ...
    #dbn2 = DBN(GaussianRBM(num_visibles=data_train_2.shape[0], num_hiddens=2))

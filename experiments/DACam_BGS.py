...
def f_measure(fg,gt):
    ...
    F = 2 * DR * percision / (DR + percision) #this part is error 
    return F
...
def main(argv):
    ...
    DIR1 = "../data/DAcam/sample1"
    data_train_0,data_train_1,data_train_2, data_test = load_data_BS(DIR1 + "/depthPKL.pkl")
    img = Image.open(DIR1 + "/groundtruth.jpg")
    print 'fin!\n'
    
    print data_train_0.shape
    x_, y_ = (np.array(img)[:,:]).shape #determine procesing image size
    print x_," and ",y_
    print "Doie : " , x_,y_
